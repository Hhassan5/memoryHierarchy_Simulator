...
bool readFile(const string &fileName, vector<unsigned long long> &addresses)
{
    ifstream inFile(fileName);
    if (!inFile)
    {
        cerr << "file error in : " << fileName << endl;
        return false;
    }
...
void simulateCache(const vector<unsigned long long> &addresses, vector<CacheLine> &cache, int memoryBits, int memoryCycles, int cacheCycles, int S, int L)
{
    int totalAccesses = 0;
    int hits = 0;
    int misses = 0;
    ...
    for (auto addr : addresses)
    {
        ...
        cout << "Tag:             " << tag << " (Binary: " << bitset<40>(tag).to_string().substr(40 - tagBitLength) << ")" << endl;
        if (isHit)
        {
...
int main()
{
    ...
    if (!readFile(instructionFileName, instructionAddresses))
    {
        return 1;
    }
    if (!readFile(dataFileName, dataAddresses))
    {
        return 1;
    }
